...
document.querySelector('.KreirajButton').addEventListener('click', async function() {
    ...
    for (let i = 1; i <= numberOfTasks; i++) {
        ...
        if (generisiRadio && generisiRadio.checked) {
            ...
            const url = `http://localhost:5246/Zadatak/Oblast/${selectedOblastId}`;
    ...
    Promise.all(promises).then(() => {
        console.log('Svi zadaci:', questions);
        ...
        const blanketDto = {
            ...
            Putanja: "path/pismeni.pdf", // Putanja do PDF-a
            Predmet: {
                Id: predmetID,
            ...
            Zadaci: questions.map(q => ({ Id: q.id, Tekst: q.tekst }))
        };
